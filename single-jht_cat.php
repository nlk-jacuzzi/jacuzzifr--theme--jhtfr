...
?>>
    	<div class="wrap">
            ...
            <div class="tub-grid">
            <?php if ( $hasmore ) {
				echo '<div class="more"><a href="#"><span>Plus d\'information </span><span class="plus">+</span></a></div>';
            	...
            	<div class="side">
					...
					<div class="tub-brochure-pricing">
						<div>
							<a class="gold-button-208" href="<?php echo get_permalink(3745); ?>"><span class="the-content">Brochure gratuite<span>></span></span></a>
						</div>
						<div class="pages40">
							<img src="<?php echo get_stylesheet_directory_uri(); ?>/images/callouts/40-pages-thumb.png" alt="40 pages de fait et photos" />
						</div>
					</div>
							...
							if ( $showlink ) {
								...
								if ( isset($c['subcats']) ) {
									...
									foreach ( $c['subcats'] as $i => $s ) {
										echo '<li'. ($i==$post->ID ? ' class="current"' : '') .'><a href="'. $s['url'] .'">'. str_replace('Collection ', '', $s['name']) .'</a>';
									}
									echo '</ul>';
								}
								echo '</li>';
						...
						if ( in_array($post->ID, array($usid, $caid)) ) { // HOT TUBS landing page
							foreach ( $tubcats as $k => $c ) {
								...
								<div class="inner htl">
									...
									<p><a href="<?php echo esc_url($c['url']); ?>">Voir les spas</a></p>
								</div>
								<?php
								echo '</li>';
							}
							...
							?>
								<div class="inner">
									<h2><?php esc_attr_e($t['name']); ?><sup>MC</sup></h2>
									<p>Nombre de places: <?php esc_attr_e(strtolower($t['seats'])) ?></p>
									<p>Jets: <?php echo absint($t['jets']) ?></p>
									<p><a href="<?php echo esc_url($t['url']); ?>" class="grey-gold-btn">Voir tous les d&eacute;tails</a></p>
								</div>
							<?php
								echo '</li>';
								}
